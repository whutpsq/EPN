...
class highwayTrajDataset(Dataset):
    ...
    def getVelc(self, dsId, vehId, t):
        colIndex = np.where(self.Tracks[dsId - 1][vehId - 1][0, :] == t)[0][0]
        vel_acc = self.getVelcByCol(dsId, vehId, colIndex)
        return vel_acc
    def getVelcByCol(self, dsId, vehId, colIndex):
        return self.Tracks[dsId - 1][vehId - 1][9:10, colIndex]
    ...
    def getdifVel(self, dsId, vehId, refVehId, t, wholePeriod=False):
        if vehId == 0:
            # if return empty, it denotes there's no vehicle in that grid.
            return np.empty([0, 1])
        else:
            vehColIndex = np.where(self.Tracks[dsId - 1][vehId - 1][0, :] == t)[0][0]
            refColIndex = np.where(self.Tracks[dsId - 1][refVehId - 1][0, :] == t)[0][0]
            vehTrackvel = self.Tracks[dsId - 1][vehId - 1][9:10].transpose()
            refTrackvel = self.Tracks[dsId - 1][refVehId - 1][9:10].transpose()
            # Use the sequence of trajectory or just the last instance as the refPos
            if wholePeriod:
                refStpt = np.maximum(0, refColIndex - self.t_h)
                refEnpt = refColIndex + 1
                refPos = refTrackvel[refStpt:refEnpt:self.d_s, :]
            else:
                refPos = np.tile(refTrackvel[refColIndex, :], (self.hist_len, 1))
            stpt = np.maximum(0, vehColIndex - self.t_h)
            enpt = vehColIndex + 1
            vehPos = vehTrackvel[stpt:enpt:self.d_s, :]
            if len(vehPos) < self.hist_len:
                histPart = vehPos-refPos[-len(vehPos)::]
                paddingPart = np.tile(histPart[0, :], (self.hist_len - len(vehPos), 1))
                hist = np.concatenate((paddingPart, histPart), axis=0)
    ...
    def getVel(self, dsId, vehId, refVehId, t, wholePeriod=False):
        if vehId == 0:
            # if return empty, it denotes there's no vehicle in that grid.
            return np.empty([0, 1])
        else:
    ...
    def getAcc(self, dsId, vehId, refVehId, t, wholePeriod=False):
        if vehId == 0:
            # if return empty, it denotes there's no vehicle in that grid.
            return np.empty([0, 1])
        else:
            vehColIndex = np.where(self.Tracks[dsId - 1][vehId - 1][0, :] == t)[0][0]
            refColIndex = np.where(self.Tracks[dsId - 1][refVehId - 1][0, :] == t)[0][0]
            vehTrackvel = self.Tracks[dsId - 1][vehId - 1][10:11].transpose()
            refTrackvel = self.Tracks[dsId - 1][refVehId - 1][10:11].transpose()
            # Use the sequence of trajectory or just the last instance as the refPos
            if wholePeriod:
                refStpt = np.maximum(0, refColIndex - self.t_h)
                refEnpt = refColIndex + 1
    ...
    def __getitem__(self, idx):
        ...
        for id, target in enumerate(centGrid):
            if target:
                ...
                vel_hist.append(self.getVel(dsId, target, target, t))
                acc_hist.append(self.getAcc(dsId, target, target, t))
                # Targets maneuvers
                ...
                nbsVel.append([self.getVel(dsId, i, target, t, wholePeriod=True) for i in grid])
                nbsAcc.append([self.getAcc(dsId, i, target, t, wholePeriod=True) for i in grid])
                nbsdifVel.append([self.getdifVel(dsId, i, target, t, wholePeriod=True) for i in grid])
                # PlanVeh future
        ...
        return planFuts, nbsHists, \
               ...
               centGrid, planGridLocs, idx , nbsVel, nbsAcc ,nbsdifVel, vel_hist, acc_hist
    ...
    def collate_fn(self, samples):
        targs_batch_size = 0
        nbs_batch_size = 0
        for _, nbsHists, targsHists, _, _, _, _, _, _, _, _, _,_ ,_ in samples:
        ...
        nbsVel_batch = torch.zeros(self.hist_len, nbs_batch_size, 1)
        nbsAcc_batch = torch.zeros(self.hist_len, nbs_batch_size, 1)
        nbsColl_batch = torch.zeros(self.hist_len, nbs_batch_size, 1)
        vel_hist_batch = torch.zeros(self.hist_len,  targs_batch_size, 1)
        acc_hist_batch = torch.zeros(self.hist_len,  targs_batch_size, 1)
        coll_hist_batch = torch.zeros(self.hist_len,  targs_batch_size, 1)
        ...
        targsEncMask_batch = torch.zeros(targs_batch_size, self.grid_size[1], self.grid_size[0], self.enc_size, dtype=self.mask_num_type)
        targsFutMask_batch = torch.zeros(self.fut_len,     targs_batch_size,  2)
        targetCount = 0
        nbCount = 0
        for i, (planFuts, nbsHists, targsHists, targsFuts, targsLonEnc, targsLatEnc, centGrid, planGridLocs, idx, nbsVel, nbsAcc, nbsdifVel,vel_hist, acc_hist) in enumerate(samples):
            idxs.append(idx)
            ...
            for j in range(len(targsFuts)):
                ...
                vel_hist_batch[0:len(vel_hist[j]), targetCount, 0] = torch.from_numpy(vel_hist[j][:, 0])
                acc_hist_batch[0:len(acc_hist[j]), targetCount, 0] = torch.from_numpy(acc_hist[j][:, 0])                
                ...
                for index, nbHist in enumerate(nbsHists[j]):
                    if len(nbHist) != 0:
                        ...
                        nbsVel_batch[0:len(nbHist), nbCount, 0] = torch.from_numpy(nbsVel[j][index][:, 0])
                        nbsAcc_batch[0:len(nbHist), nbCount, 0] = torch.from_numpy(nbsAcc[j][index][:, 0])
                        ...
                        if (pos[0] <= 13):
                                if (nbsdifVel[j][index][-1,0] < 0):
                                    for z in range(len(nbHist)):
                                        if(nbsdifVel[j][index][z, 0]==0):
                                            nbsColl_batch[z, nbCount, 0] = 0
                                        else:
                                            nbsColl_batch[z, nbCount, 0] = (nbHist[z,0]**2+nbHist[z,1]**2)**0.5/nbsdifVel[j][index][z, 0]
                                    # nbsColl_batch[0:len(nbHist), nbCount, 0] = torch.from_numpy((nbHist[0:len(nbHist),0]**2+nbHist[0:len(nbHist),1]**2)**0.5/nbsdifVel[j][index][:, 0])
                        if(pos[0] > 13):
                                if (nbsdifVel[j][index][-1,0] > 0):
                                    for z in range(len(nbHist)):
                                        if(nbsdifVel[j][index][z, 0]==0):
                                            ...
                                            nbsColl_batch[z, nbCount, 0] = (nbHist[z,0]**2+nbHist[z,1]**2)**0.5/nbsdifVel[j][index][z, 0]
                        nbCount += 1
                        targsEncMask_batch[targetCount, 3, 12, :] = torch.ones(self.enc_size).byte()
        ...
        return nbsHist_batch, nbsMask_batch, \
               ...
               targsFut_batch, targsFutMask_batch, lat_enc_batch, lon_enc_batch, idxs, nbsVel_batch , nbsAcc_batch, nbsColl_batch,vel_hist_batch, acc_hist_batch, coll_hist_batch
