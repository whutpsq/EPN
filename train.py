...
def train_model():
    ...
    if args.use_cuda:
        EPN = EPN.cuda()
    optimizer = torch.optim.Adam(EPN.parameters(), lr=args.learning_rate)
    ...
    for epoch_num in range( pretrainEpochs + trainEpochs ):
        ...
        for i, data in enumerate(trDataloader):
            ...
            if args.use_cuda:
                ...
                dest = dest.cuda()
            # Forward pass
            fut_pred, lat_pred, lon_pred, generated_dest, mu, logvar = EPN(nbsHist, nbsMask, planFut, planMask, targsHist, targsEncMask, lat_enc, lon_enc, nbsVel, nbsAcc, nbsColl,vel_hist,acc_hist,coll_hist,dest)
            l = maskedMSE(fut_pred, targsFut, targsFutMask)+0.5*maskedDestMSE(generated_dest,dest,targsFutMask)+0.5*KL(mu,logvar)

            # Back-prop and update weights
            optimizer.zero_grad()
            l.backward()
            prev_vec_norm = torch.nn.utils.clip_grad_norm_(EPN.parameters(), 10)
            optimizer.step()
                ...
                with torch.no_grad():
                    ...
                    for i, data in enumerate(valDataloader):
                        ...
                        # if epoch_num < pretrainEpochs:
                            ...
                            EPN.train_output_flag = True
                            fut_pred, _, _,_,_ ,_= EPN(nbsHist, nbsMask, planFut, planMask, targsHist, targsEncMask, lat_enc, lon_enc, nbsVel, nbsAcc, nbsColl,vel_hist,acc_hist,coll_hist,dest)
        ...
        if epoCount < pretrainEpochs:
            torch.save(EPN.state_dict(), log_path + "{}-pre{}-nll{}.tar".format(args.name, epoCount, 0))
        else:
            torch.save(EPN.state_dict(), log_path + "{}-pre{}-nll{}.tar".format(args.name, pretrainEpochs, epoCount - pretrainEpochs))

    # All epochs finish________________________________________________________________________________________________
    torch.save(EPN.state_dict(), log_path+"{}.tar".format(args.name))
